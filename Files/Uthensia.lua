...
log.warning("ENJOY MODDING !!")
    ...
    INTRXDUCE = gui.get_tab("UTHENSIA POR INTRXDUCE")
    INTRXDUCE:add_text("                      UTHENSIA MENU 1.72 ")
    INTRXDUCE:add_text("                          I N T R X D U C E ")
    INTRXDUCE:add_text("                    TRADUCIDO POR BLUEXB ")


    moneymethods = INTRXDUCE:add_tab("METODOS DE DINERO")
    ranksmenu = INTRXDUCE:add_tab("NIVEL", {color = {255, 0, 0}})
    unlocks = INTRXDUCE:add_tab("DESBLOQUEABLES")


    
VehiclesMenu = INTRXDUCE:add_tab("MENU DE AUTOS")

VehiclesMenu:add_button("ACTIVAR VEHICULOS ELIMINADOS", function()
    ...
    MissionEdit = INTRXDUCE:add_tab("EDITOR DE MISION")

    MissionEdit:add_button("ELIMINAR MISIONES DE DAX FOOLIGAN ", function()
	stats.set_int(MPX() .. "XM22JUGGALOWORKCDTIMER", -1)
end)

MissionEdit:add_button("QUITAR ENFRIAMIENTO DE VIP/MC", function()
	stats.set_int("MPPLY_VIPGAMEPLAYDISABLEDTIMER", 0)
end)

MissionEdit:add_button("SALTAR MISIONES DEL YATE", function()
...
MissionEdit:add_separator()
    ...
    RankEditor = ranksmenu:add_tab("EDITOR DE NIVEL")

    RankEditor:add_text("MAX 10 NIVELES CADA 24 HORAS")
    RankEditor:add_separator()
    
    RankEditor:add_text("SETEA TU NIVEL:")
    
    local rankLevel = RankEditor:add_input_int("NIVEL")
    
    RankEditor:add_button("SETEAR NIVEL", function()
        local levelValue = rankLevel:get_value()
    
        if levelValue <= 0 or levelValue > 8000 then
            gui.show_message(
                "ERROR",
                "YOUR RP LEVEL " .. levelValue .. " CANNOT BE SET BECAUSE IT IS OUTSIDE THE VALID RANGE (1-8000)."
            )
        else
            stats.set_int(MPX() .. "CHAR_SET_RP_GIFT_ADMIN", levels[levelValue] or 0)
            globals.set_int(1575036, 11)
            globals.set_int(1574589, 1)
            globals.set_int(1574589, 0)
            gui.show_message(
                "RANK EDITOR",
                "YOUR RANK IS SET TO " .. levelValue .. ", CHANGE SESSION TO APPLY."
            )
        end
    end)
    
    CrewRankEditor = ranksmenu:add_tab("EDITOR DE NIVEL (CREW)")


    CrewRankEditor:add_text("SETEA TU NIVEL DE CREW:")
    
    local rankLevel = CrewRankEditor:add_input_int("NIVEL")
    
    CrewRankEditor:add_button("SETEAR NIVEL DE CREW", function()
        local levelValue = rankLevel:get_value()
    
        if levelValue <= 0 or levelValue > 8000 then
            gui.show_message(
                "ERROR",
                "YOUR CREW LEVEL " .. levelValue .. " CANNOT BE SET BECAUSE IT IS OUTSIDE THE VALID RANGE (1-8000)."
            )
        else
            stats.set_int("MPPLY_CREW_LOCAL_XP_0", levels[levelValue] or 0)
            stats.set_int("MPPLY_CREW_LOCAL_XP_1", levels[levelValue] or 0)
            stats.set_int("MPPLY_CREW_LOCAL_XP_2", levels[levelValue] or 0)
            stats.set_int("MPPLY_CREW_LOCAL_XP_3", levels[levelValue] or 0)
            stats.set_int("MPPLY_CREW_LOCAL_XP_4", levels[levelValue] or 0)
            gui.show_message("CREW RANK EDITOR", "YOUR CREW RANKS ARE NOW " .. levelValue .. ", .")
        end
    end)
    
    LatestUnlock = unlocks:add_tab("DESBLOQUEAR TODO")
    LSCMUnlocker = unlocks:add_tab("DESBLOQUEAR LSCM")
    ...
    LatestUnlock:add_button("DESBLOQUEAR TODO", function()
    ...
    LatestUnlock:add_button("ESTADISTICAS RELACIONADAS AL TIEMPO", function()
        ...
        gui.show_message(
			"UNLOCKER",
			"ESTADISTICAS RELACIONADAS AL TIEMPO DESBLOQUEADAS"
)
    end)
    LatestUnlock:add_separator()
    LatestUnlock:add_button("BOLOS", function()
        ...
        gui.show_message(
			"UNLOCKER",
			"BOOLS ARE NOW UNLOCKED"
)
    end)
    
    LatestUnlock:add_separator()

    LatestUnlock:add_button("TRAJES DEL DIAMOND CASINO", function()
        ...
        gui.show_message(
			"UNLOCKER",
			"TRAJES DEL DIAMOND CASINO DESBLOQUEADOS"
)
    end)

    LatestUnlock:add_separator()
    
    LatestUnlock:add_button("PRECIO ESPECIAL OPPRESSOR MKII", function()
        stats.set_masked_int(MPX() .. "BUSINESSBATPSTAT_INT379", 5, 5, 5) --Pegassi Oppressor Mk II (Trade Price)
        gui.show_message(
			"UNLOCKER",
			"PRECIO ESPECIAL OPPRESSOR MKII DESBLOQUEADO"
)
    end)
    
    LatestUnlock:add_separator()

    LatestUnlock:add_button("TODOS LOS TATUAJES", function()
    ...
    LatestUnlock:add_button("TATUAJES DE CALAVERAS", function()
        stats.set_bool(MPX() .. "AWD_500_HEADSHOTS", true)
        gui.show_message(
			"UNLOCKER",
			"TATUAJES DE CALAVERAS DESBLOQUADOS"
)
    end)

    LatestUnlock:add_separator()

    LSCMUnlocker:add_button("REPUTACION NIVEL 1000", function()
        for i = 262145 + 30958, 262145 + 30987 do
            globals.set_float(i, 100000)
            gui.show_message(
                "UNLOCKER",
                "LSCM NIVEL DE REP ACTUAL 1000"
    )
        end
    end)
    ...
    LSCMUnlocker:add_button("PASEO DE PREMIOS LSCM", function()
        stats.set_bool(MPX() .. "CARMEET_PV_CHLLGE_CMPLT", true)
        gui.show_message(
			"UNLOCKER",
			"LSCM - PASE DE PREMIOS DESBLOQUEADO"
)
    end)

    LSCMUnlocker:add_separator()

    LatestUnlock:add_button("HABILIDADES MAXIMAS", function()
        stats.set_int(MPX() .. "SCRIPT_INCREASE_DRIV", 100)
        stats.set_int(MPX() .. "SCRIPT_INCREASE_FLY", 100)
        stats.set_int(MPX() .. "SCRIPT_INCREASE_LUNG", 100)
        stats.set_int(MPX() .. "SCRIPT_INCREASE_SHO", 100)
        ...
        gui.show_message(
			"UNLOCKER",
			"HABILIDADES DEL PERSONAJE MAXEADAS"
)
    end)

    LatestUnlock:add_separator()

    LatestUnlock:add_button("PRECIO ESPECIAL COCHES POLICIALES", function()
        ...
        gui.show_message(
			"UNLOCKER",
			"PRECIO ESPECIAL COCHES POLICIALES DESBLOQUEADOS"
)
    end)
    
    LatestUnlock:add_separator()

    LatestUnlock:add_button("REGALOS DE NAVIDAD 2023", function()
        globals.set_int(262145 + 35157, 1) --XMASGIFTS2023
        globals.set_int(262145 + 35158, 1) --NEWYEARSGIFTS2023
        gui.show_message(
			"UNLOCKER",
			"REGALOS DE NAVIDAD 2023 DESBLOQUEADO"
)
    end)
    
    LatestUnlock:add_separator()

    LatestUnlock:add_button("DISEÑO DE TAXI POR EUDORA", function()
        stats.set_masked_int(MPX() .. "DLC22022PSTAT_INT536", 10, 16, 8)
        gui.show_message(
			"UNLOCKER",
			"DISEÑO DE TAXI POR EUDORA DESBLOQUEADO"
)
    end)
    
    LatestUnlock:add_separator()

    LatestUnlock:add_button("DISEÑO DE TAXI BROADWAY", function()
        stats.set_int(MPX() .. "AWD_TAXIDRIVER", 50)
        gui.show_message(
			"UNLOCKER",
			"DISEÑO DE TAXI BROADWAY DESBLOQUEADO"
)
    end)
    ...
    function TransactionManager.new()
        ...
        instance.Transactions = {
            {label = "15 MILLONES (Detectado)", hash = 0xA174F633},
            {label = "15 MILLONES (Detectado)", hash = 0xA174F633},
            {label = "7 MILLONES (Muy Riesgoso)", hash = 0xED97AFC1},
            {label = "3.6 MILLONES", hash = 0xB703ED29},
            {label = "2.5 MILLONES", hash = 0x46521174},
            {label = "2.5 MILLONNES", hash = 0xDBF39508},
            {label = "2 MILLONES", hash = 0x8107BB89},
            {label = "2 MILLONES", hash = 0x921FCF3C},
            {label = "2 MILLONES", hash = 0x4B6A869C},
            {label = "1 MILLONES", hash = 0xE9BBC247},
            {label = "1 MILLONES", hash = 0x314FB8B0},
            {label = "1 MILLONES", hash = 0xBFCBE6B6},
            {label = "680 MIL", hash = 0xACA75AAE},
            {label = "500 MIL", hash = 0x05F2B7EE},
            {label = "310 MIL", hash = 0xEE884170},
            {label = "200 MIL", hash = 0xBA16F44B},
            {label = "200 MIL",  hash = 0x9145F938},
            {label = "200 MIL",    hash = 0xCDCF2380},
            {label = "190 MIL",   hash = 0xFD389995}
        }
    ...
    function TransactionManager:Init()
        local sub_transactionL  = moneymethods:add_tab("METODO DE DINERO OP")
    ...
    NightClubMoney = moneymethods:add_tab("CLUB NOCTURNO SAFE LOOP")
    ...
    NCRSCB = NightClubMoney:add_checkbox("250K SAFE LOOP")
    ...
    NightClubMoney:add_separator()
    NightClubMoney:add_text("POSIBLEMENTE DETECTADO")


    KDS = INTRXDUCE:add_tab("EDITOR DE K/D")

    TypeKd = KDS:add_tab("CUSTOMIZAR K/D")
    
    KdValue = TypeKd:add_input_float("SETEA TU K/D")
    
    TypeKd:add_button("SETEAR K/D", function()
        ...
        if kd > 0 then
            ...
            gui.show_message(
                "K/D ACTUALIZADO",
                "TU K/D AHORA ES " .. kd .. " . MIRA TUS ESTADISTICAS!"
            )
    ...
    TypeKd:add_text(" NO USAR (EXTREMADAMENTE RIESGOSO)")
    TypeKd:add_separator()

    KillDeathEdit = KDS:add_tab("EDITOR DE KILLS/DEATHS")
    
    KillValue = KillDeathEdit:add_input_int("SETEA TUS KILLS")
    DeathValue = KillDeathEdit:add_input_int("SETEA TUS DEATHS")
    
    KillDeathEdit:add_button("SETEA KILLS Y DEATHS", function()
        ...
        stats.set_int("MPPLY_DEATHS_PLAYER", deaths)
        gui.show_message(
                "EDITOR DE KILLS/DEATHS",
                "YOUR KILLS/DEATHS IS NOW SET TO " .. kills .. "/" .. deaths .. ", CHECK YOUR STATS TO BE SURE!"
    )
    end)

    KillDeathEdit:add_text(" NO USAR (EXTREMADAMENTE RIESGOSO)")
    KillDeathEdit:add_separator()
    
OneKd = KDS:add_tab("K/Ds CON UN CLICK")

PLUS = OneKd:add_tab("POSITIVO K/D (RIESGOSO)")
...
MINUS = OneKd:add_tab("NEGATIVO K/D (RIESGOSO)")
    ...
    CheckUrReports = INTRXDUCE:add_tab("REPORTES")
    CheckUrReports:add_text("GRIFEOS: " .. stats.get_int("MPPLY_GRIEFING"))
    CheckUrReports:add_text("EXPLOITS: " .. stats.get_int("MPPLY_EXPLOITS"))
    CheckUrReports:add_text("ABUSAR DE BUGS: " .. stats.get_int("MPPLY_GAME_EXPLOITS"))
    CheckUrReports:add_text("MOLESTAR PERSONAS EN EL CHAT: " .. stats.get_int("MPPLY_TC_ANNOYINGME"))
    CheckUrReports:add_text("MENSAJES DE ODIO EN EL CHAT: " .. stats.get_int("MPPLY_TC_HATE"))
    CheckUrReports:add_text("MOLESTAR PERSONAS EN CHAT DE VOZ: " .. stats.get_int("MPPLY_VC_ANNOYINGME"))
    CheckUrReports:add_text("MENSAJES DE ODIO EN CHAT DE VOZ: " .. stats.get_int("MPPLY_VC_HATE"))
    CheckUrReports:add_text("LENGUAJE OFENSIVO: " .. stats.get_int("MPPLY_OFFENSIVE_LANGUAGE"))
    CheckUrReports:add_text("NOMBRE OFENSIVO: " .. stats.get_int("MPPLY_OFFENSIVE_TAGPLATE"))
    CheckUrReports:add_text("CONTENIDO OFENSIVO: " .. stats.get_int("MPPLY_OFFENSIVE_UGC"))
    CheckUrReports:add_text("NOMBRE DEL CREW OFENSIVO: " .. stats.get_int("MPPLY_BAD_CREW_NAME"))
    CheckUrReports:add_text("NOMBRE DEL CLUB DE MOTEROS OFENSIVO: " .. stats.get_int("MPPLY_BAD_CREW_MOTTO"))
    CheckUrReports:add_text("MAL ESTADO DE LA CREW: " .. stats.get_int("MPPLY_BAD_CREW_STATUS"))
    CheckUrReports:add_text("MAL EMBLEMA DE LA CREW: " .. stats.get_int("MPPLY_BAD_CREW_EMBLEM"))
    CheckUrReports:add_separator()
    CheckUrReports:add_text("ELOGIOS/RECOMENDACIONES:")
    CheckUrReports:add_separator()
    CheckUrReports:add_text("AMISTOSO: " .. stats.get_int("MPPLY_FRIENDLY"))
    CheckUrReports:add_text("UTIL: " .. stats.get_int("MPPLY_HELPFUL"))
...
local VehiclesMenu = VehiclesMenu:add_button("RECLAMAR VEHICULO ACTUAL COMO PERSONAL", function()
        ...
        if network.is_session_started() and script.is_active("freemode") and script.is_active("am_mp_vehicle_reward") then
            if PED.IS_PED_IN_ANY_VEHICLE(self.get_ped(), false) then
                if IS_VEHICLE_VALID_FOR_PV(ENTITY.GET_ENTITY_MODEL(self.get_veh())) then
                    ...
                    gui.show_error("VEHICLE MENU", "ESTE COCHE NO SE PUEDE GUARDAR COMO VEHICULO PERSONAL")
                end
            else
                gui.show_error("VEHICLE MENU", "PORFAVOR OBTEN UN COCHE.")
            end
        else
            gui.show_error("VEHICLE MENU", "NO PUEDES GUARDAR EL COCHE EN ESTE MOMENTO.")
        end
    end)
end)
...
ilovecredits = INTRXDUCE:add_tab("C R E D I T O S")
    ...
    BLUEXB = ilovecredits:add_tab("BLUEXB")
    BLUEXB:add_text("TRADUCCION ESPAÑOL")
