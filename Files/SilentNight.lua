...
GSIg = 1673807 		-- get supplies instantly global ("OR_PSUP_DEL)
...
NLCl = 206 + 32 + 1 -- night loop collect local
...
Silent:add_text("TRADUCIDO A ESPAÑOL POR BLUEXB")
Silent:add_text("TU NIVEL DE RP ES " .. stats.get_int(joaat(MPX() .. "CHAR_RANK_FM")))
Silent:add_text("TU NICEL DE CREW ES " .. stats.get_int(joaat("MPPLY_CURRENT_CREW_RANK")))
...
local Manager = Silent:add_tab("Jugador ")
local player = Manager:add_tab("Nivel del jugador ")
player:add_text("Tu nivel avtual es ".. stats.get_int(joaat(MPX() .. "CHAR_RANK_FM")))
local globalFix = player:add_checkbox("Fix nivel global")
local rpLevel = player:add_input_int("Nivel")
player:add_button("Setear Nivel",
function ()
	if rpLevel:get_value() > 8000 or rpLevel:get_value() < 0 then
		gui.show_message("ERROR", "Tu nivel es ".. rpLevel:get_value().." ESTA POR ENCIMA O DEBAJO DEL LIMITE")
	else
		if rpLevel:get_value() >= 98 then
			...
			gui.show_message("RP", "TU NIVEL SE SETEO EN "..rpLevel:get_value()..", CAMBIA DE SESION...")
			changeSession(8)
		else
...
local crew = Manager:add_tab("Nivel de Crew ")
crew:add_text("Tu nivel de crew es ".. stats.get_int(joaat("MPPLY_CURRENT_CREW_RANK")))
local level = crew:add_input_int("Nivel")
crew:add_button("Setear Nivel",
function()
...
local skills = Manager:add_tab("Habilidades ")
skills:add_button("Maxear Habilidades",
function ()
...
skills:add_sameline()
skills:add_button("Resetear habilidades del jugador",
function ()
...
-- Heist Editor tab
Heist_Editor = Silent:add_tab("Editor De Golpes ")
...
local sContract = 0
local Agency = Heist_Editor:add_tab("Agencia ")
Agency:add_imgui(function()
    ImGui.Text("Contratos:")
...
Agency:add_button("Completar Preparatorias", function()
...
Agency:add_text("Dinero")
local agencySafe = Agency:add_checkbox("Loop Seguro Agencia")
...
Agency:add_separator()
Agency:add_text("Pagos:")
Agency:add_text("Establece el pago despues de iniciar la mision final:")
Agency:add_button("Max Payout", function() globals.set_int(APg, 2500000) end)
Agency:add_text("")
Agency:add_text("Pago customizado:")
local agencyPayout = Agency:add_input_int("Pago")
Agency:add_button("Setear Pago", function() globals.set_int(APg, agencyPayout:get_value()) end)
Agency:add_separator()
...
script.register_looped("removeCool", function(remt)
    ...
    if remCD:is_enabled() == true then
		globals.set_int(1877158 + 1 + (PLAYER.PLAYER_ID() * 77) + 76, -1)
		remt:sleep(500)
	end
end)
...
script.run_in_fiber(function(ap)
	local key = globals.get_int(1877158 + (PLAYER.PLAYER_ID() * 77) + 24 + 2)
	local cuts = heistCuts[key]
	globals.set_int(ACg1, 100 - (cuts[2]*locals.get_int("fmmc_launcher", 20054 + 34)))
    ...
    function ()
        ...
        if ImGui.Button("Set Cuts") then
            script.run_in_fiber(function(ccut)
                local heistType = locals.get_int("fmmc_launcher", 20054 + 34)
...
Apartment:add_button("Instant Finish (Pacific)", function()
    ...
    locals.set_int("fm_mission_controller", 19791 + 1740 + 1, 80)
...
local MoneyT = Silent:add_tab("Dinero ")

-- Bunker Crash --

local Bunker = MoneyT:add_tab("Bunker Crash ")
Bunker:add_text("Rellenar suministros")
Bunker:add_button("Rellenar",
function ()
	globals.set_int(GSIg + 6, 1)
end
)
Bunker:add_button("Abrir Laptop", function()
...
Bunker:add_text("Desbloquear toda la investigación temporalmente")

Bunker:add_button("Desbloquear", function()
	script.run_in_fiber(function(script)
		globals.set_int(FMg + 21564, 1)
		gui.show_message("Bunker Research", "ALL Bunker research has been unlocked.")
	end)
end)
Bunker:add_text("Suministros")
Bunker:add_button("Produccion Rapida", function()
...
Bunker:add_sameline()
local bSupplies = Bunker:add_checkbox("Reponer Bunker (Loop - Inseguro)")
...
Bunker:add_text("Terminar misiones instantaneo")
Bunker:add_button("Vender suministros instantaneo",
function ()
...
MB:add_text("Reabastecer a todas las empresas")
MB:add_button("Reabastecer todo", function()
...
MB:add_sameline()
MB:add_button("Produccion rapida", function()
...
MB:add_separator()
MB:add_text("Aumentar el valor del producto a $1,000,000 +")
MB:add_button("Aumentar los precios de venta", function()
...
Hangar:add_text("Suministros")
Hangar:add_button("Dile a Rooster que consiga algunos suministros de carga",
function ()
	stats.set_packed_stat_bool(36828, true)
end
)
Hangar:add_button("Abrir Laptop", function()
    SCRIPT.REQUEST_SCRIPT("appsmuggler")
    SYSTEM.START_NEW_SCRIPT("appsmuggler", 4592)
end)

local hangarloop = Hangar:add_checkbox("Loop Hangar (Inseguro)")
...
local hangarMaxPrice = Hangar:add_checkbox("Maximizar precio")
...
Hangar:add_separator()
local HangarCargo = 0
Hangar:add_text("Misiones de venta de carga instantaneas")
HangarCargo = Hangar:add_input_int(" Cargos")
Hangar:add_button("Terminar Instantaneo",
function ()
	script.run_in_fiber(
		function (script)
...
local Nightclub = MoneyT:add_tab("Utilidades Club Nocturno ")
Nightclub:add_text("Club Nocturno Loop (Semi-Seguro)")
Nightclub:add_text("Esto llena la popularidad del club nocturno y llena la caja fuerte con 50 000 cada 2 segundos y se recoge automaticamente")
local safeloop = Nightclub:add_checkbox("Habilite Nitghtclub $250k/15s (AFK semi-seguro)")
count = 0
...
Nightclub:add_text("Club Nocturno (Seguro)")
Nightclub:add_button("Rellenar Club Nocturno",
function ()
	script.run_in_fiber(
		function (script)
...
Nightclub:add_button("Recoger Ganancias",
function ()
	locals.set_int("am_mp_nightclub", NLCl, 1)
end
)
...
Nightclub:add_text("Setear valor del cargamento a $2,000,000")
...
Nightclub:add_text("")
Nightclub:add_button("Pagos Default", function()
...
local nightclubPopLock = Nightclub:add_checkbox("Bloquear Popularidad")
...
Nightclub:add_sameline()
local nightclubCooldown = Nightclub:add_checkbox("Remover Enfriamientos")
...
arcadeSafe = arcade:add_checkbox("Arcade (Loop Seguro)")
...
local Special = MoneyT:add_tab("Carga especial ")
Special:add_text("Obtener Cargas de Suministros")
Special:add_button("Obtener Suministros",
function ()
	stats_set_packed_bools(32359, 32363, true)
end
)
Special:add_separator()
Special:add_text("Cajas de carga")
local cargoloop = Special:add_checkbox("Loop de suministros de carga")
...
Special:add_separator()
Special:add_text("Loop de carga manual")

local cratevalue = 10000
Special:add_imgui(function()
    cratevalue, used = ImGui.SliderInt("Value", cratevalue, 10000, 5000000)
    if used then
        for i = 0, 20, 1 do
...
Special:add_separator()
Special:add_text("Vender instantaneamente")
Special:add_button("Venta Instantanea",
function()
	script.run_in_fiber(
		function (script)
...
-- Easy Money --
local EasyMoney = MoneyT:add_tab("Dinero Facil ")
...
EasyMoney:add_text("PRECAUCION: EXTREMADAMENTE INSEGURO")

-- Miscellaneous --
local Miscellaneous = Silent:add_tab("Misc ")
-- Vehicle Utilities --
...
local VehUtils = Miscellaneous:add_tab("Utilidades del Vehiculo ")
VehUtils:add_text("Setear velocidad maxima")
local vehTopSpeed = VehUtils:add_input_int("Velocidad Maxima")
VehUtils:add_button("Setear velocidad maxima",
function ()
	FastAcc(vehTopSpeed:get_value() - 100)
end
)
local fastAcc = VehUtils:add_checkbox("Loop de aceleracion rapida")
...
VehUtils:add_text("Desbloqueos de vehiculos")
VehUtils:add_button("Desbloquear",
function ()
...
VehUtils:add_button("Desbloquear vehiculos removidos",
function ()
...
local unlocker = Miscellaneous:add_tab("Menu de desbloqueo ")
unlocker:add_text("Desbloquea toda las cosas que quieras")
unlocker:add_button("Todo los paracaidas",
	function()
...
unlocker:add_button("Todos los tatuajes",
	function()
		for i = 0, 53 do
	...
	unlocker:add_sameline()
unlocker:add_button("Trajes del casino", function() stats_set_packed_bools(28225, 28248, true) end)

unlocker:add_button("Habilitar recarga y correr rapido",
function () 
	for i = 1, 3 do
		stats.set_int(MPX() .. "CHAR_FM_ABILITY_" .. i .. "_UNLCK", -1)
		stats.set_int(MPX() .. "CHAR_ABILITY_" .. i .. "_UNLCK", -1)
	end
	gui.show_message("CORRER Y RECARGA RAPIDA HABILITADA", "CAMBIA DE SESION")
end)
unlocker:add_sameline()
unlocker:add_button("Desactivar recarga y correr rapido",
function () 
	for i = 1, 3 do
		stats.set_int(MPX() .. "CHAR_FM_ABILITY_" .. i .. "_UNLCK", 0)
		stats.set_int(MPX() .. "CHAR_ABILITY_" .. i .. "_UNLCK", 0)
	end
	gui.show_message("CORRER Y RECARGA RAPIDA DESACTIVADA", "CAMBIA DE SESION")
end)
unlocker:add_sameline()
unlocker:add_button("Vehiculos de Hallooween",
function ()
	globals.set_int(262145 + 12025, 1)
end)
unlocker:add_button("POLERAS BASE5",
function() 
	globals.set_int(262145 + 23812, 1) 
end)
unlocker:add_sameline()
unlocker:add_button("Mascaras de mercenarios",function() 
	for i = 34625,34643 do
		globals.set_int(262145 + i, 1)
	end
end)
unlocker:add_sameline()
unlocker:add_button("Medalla de oro escuela de aviacion",
	function()
		for i = 0, 9 do
...
unlocker:add_button("Precios especiales coches policiales",
	function()
...
unlocker:add_sameline()
unlocker:add_button("La Shocker",
function ()
...
unlocker:add_sameline()
unlocker:add_button("Liveries ocultas",
function ()
	for i = 0, 20 do
...
unlocker:add_separator()
unlocker:add_text("Recompensas del ultimo dolar")
unlocker:add_button("Desbloquear ropas",
function ()
...
unlocker:add_sameline()
unlocker:add_button("Desbloquear mascaras",
function ()
...
unlocker:add_sameline()
unlocker:add_button("Ropa especial",
function ()
...
unlocker:add_separator()
unlocker:add_text("Deesbloquear todo los coleccionables")
function unlock_packed_bools(from, to)
    for i = from, to do
        stats.set_packed_stat_bool(i, true)
    end
end
unlocker:add_button("Bloqueador de Señal",
function ()
...
unlocker:add_sameline()
unlocker:add_button("LD Organicos",
function ()
	unlock_packed_bools(34262, 34361)
end
)
unlocker:add_sameline()
unlocker:add_button("Figuras de accion y Cartas",
function ()
	unlock_packed_bools(26811, 26964)
end
)
unlocker:add_sameline()
unlocker:add_button("Hombre de nieve",
function ()
...
local LSCM = Miscellaneous:add_tab("menu de desbloqueo LSCM ")

LSCM:add_button("Desbloquear todo - nivel 1000", function()
	for i = 262145 + 30958, 262145 + 30987 do
		globals.set_float(i, 100000)
	end
end)
LSCM:add_button("Precios especiales luces", function() stats_set_packed_bools(24980, 24991, true) end)

LSCM:add_button("Desbloquear premios",
	function()
...
local gun_van_tab = Silent:add_tab("Menu de armas VAN ")
...
gun_van_tab:add_imgui(function()
	...
	ImGui.Text("Localizacion actual: " .. (gun_van_loc ~= -1 and gun_van_loc + 1 or "N/A"))
	...
	if ImGui.Button("Setetear localizacion del van") then
		globals.set_int(2652579 + 2706, selected_loc)
	end

	ImGui.SameLine()

	if ImGui.Button("Teletransportarse") then
	...
	weapon_skins, on_tick = ImGui.Checkbox("Skin de armas", weapon_skins)
	...
	livery_lock, on_tick = ImGui.Checkbox("Quitar bloqueo de livery", livery_lock)
	help_marker(
	...
	gta_plus_blip, on_tick = ImGui.Checkbox("Señal siempre visible", gta_plus_blip)
	...
	discount, on_tick = ImGui.Checkbox("Descuento de armas (-10%)", discount)
	...
	if ImGui.TreeNode("Armas") then
		...
		if ImGui.Button("Editar Armas") then
	...
	if ImGui.TreeNode("Arrojadizos") then
		for i = 0, 2 do
	...
	if ImGui.TreeNode("Armadura") then
		...
		ImGui.Text("Armadura Super Ligera (" .. armour_discounts[1] .. "%)")
		ImGui.Text("Armadura Ligera (" .. armour_discounts[2] .. "%)")
		ImGui.Text("Armadura Standar (" .. armour_discounts[3] .. "%)")
		ImGui.Text("Armadura Pesada (" .. armour_discounts[4] .. "%)")
		ImGui.Text("Armadura Super Pesada (" .. armour_discounts[5] .. "%)")
...
-- No Needs --
local NoNeed = Silent:add_tab("No hay necesidad ")
NoNeed:add_text("Este apartado generalmente no suele funcionar")
NoNeed:add_separator()
NoNeed:add_text("OBJETIVO DE RECOMPENSAS ESTANDAR")
NoNeed:add_button("Todo con maximo pago",
function ()
...
NoNeed:add_button("Sacar a todos los prisioneros",
function ()
...
NoNeed:add_text("Nota: Hazlo fuera de la oficina de fianzas")
			


-- Credits --
local Credits = Silent:add_tab("Creditos ")
Credits:add_text("Developer: SilentSalo", null)
Credits:add_text("Traductor: BlueXB", null)
