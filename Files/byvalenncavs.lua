...
local HGWHmaxgoods = true 	-- Should the script instantly max out your Hangar and Warehouse(s) stock? - false will restock with mixed goods but it's a bit slower - The four lines below this depends on this one - True as default
...
script.run_in_callback(function()
 ...
 Main logic will start in 10s.								 
	...
	local function GetBusinessSlot(businessName)
		...
		for _, business in ipairs(MCbiz) do -- Credit to Silenthy6 (SilentSalo) for this part [url]https://www.unknowncheats.me/forum/4380348-post701.html[/url] (it was too much for my brain)
		...
		if MCrestock then
			...
			if methmc then
				if ScriptGlobal(bbizprop + 1):at(slot, 13):get_int() <= 19 then
					for _, manuprod in ipairs({17384, 17392, 17398, 17437}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({17403, 17409}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
			...
			if weedmc then
				if ScriptGlobal(bbizprop + 1):at(slot, 13):get_int() <= 79 then
					for _, manuprod in ipairs({17383, 17393, 17399, 17438}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({17404, 17410}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
			...
			if crackmc then
				if ScriptGlobal(bbizprop + 1):at(1, 13):get_int() <= 9 then
					for _, manuprod in ipairs({17385, 17391, 17397, 17436}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({17402, 17408}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
			...
			if cashmc then
				if ScriptGlobal(bbizprop + 1):at(0, 13):get_int() <= 39 then
					for _, manuprod in ipairs({17387, 17390, 17396, 17435}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({17401, 17407}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
			...
			if fakeidmc then
				if ScriptGlobal(bbizprop + 1):at(slot, 13):get_int() <= 59 then
					for _, manuprod in ipairs({17386, 17389, 17395, 17434}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({17400, 17406}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
			...
			if bunkermc then
				if ScriptGlobal(bbizprop + 1):at(slot, 13):get_int() <= 99 then
					for _, manuprod in ipairs({21341, 21343, 21344}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({21345, 21346}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
			...
			if ScriptGlobal(bbizprop):at(6, 13):get_int() >= 1 then
				if ScriptGlobal(bbizprop + 1):at(6, 13):get_int() <= 159 then
					for _, manuprod in ipairs({17388, 17394, 17439}) do
						ScriptGlobal(262145 + manuprod):set_int(1)
					end
					for _, manucost in ipairs({17405, 17411}) do
						ScriptGlobal(262145 + manucost):set_int(1)
					end
					script.yield(2000)
		...
		if ScriptGlobal(ncprop):get_int() >= 1 then
			...
			if NCgoods then
				for _, manuprod in ipairs({24040, 24041, 24042, 24043, 24044, 24045, 24046}) do
					ScriptGlobal(262145 + manuprod):set_int(1)
				end
				notify.success("Success!", "Nightclub stock have been replenished!")
			end
			script.yield(2000)
		else
